...
class EnhancedSmartKnowledgeController {
    constructor() {
        ...
        this.arabicStemmer = newStemmer('arabic');
    ...
    async findBestMatch(req, res) {
        const userInput = req.body.message || req.body.text; // Support different request bodies
        ...
        try {
            ...
            // 2. RUN PARALLEL SEARCHES IN A SINGLE AGGREGATION
            const searchResults = await Knowledge.aggregate([
                {
                    ...
                    $vectorSearch: {
                        ...
                        numCandidates: 150,
                        limit: 5, // Get top 5 semantic candidates
                        filter: { language: { $eq: userLang } }
                    }
                },
                {
                    // Add a field to identify these as vector results and store their rank
                    $addFields: {
                        search_type: 'vector',
                        vector_rank: { $indexOfArray: ["$$ROOT", "$$ROOT"] } // Hacky way to get rank
                    }
                },
                {
                    // Stage 2: Union with Text Search results
                    $unionWith: {
                        coll: 'knowledges', // The name of your collection
                        pipeline: [
                            {
                                $search: {
                                    index: 'default',
                                    compound: {
                                        must: [{
                                            text: { query: userInput, path: ['text', 'keywords'], fuzzy: {} }
                                        }],
                                        filter: [{ term: { path: 'language', query: userLang } }]
                                    }
                                }
                            },
                            { $limit: 5 }, // Get top 5 keyword candidates
                            { $addFields: { search_type: 'text' } }
                        ]
                    }
                },
                {
                    // Stage 3: Group by document ID to handle duplicates
                    $group: {
                        _id: "$_id",
                        doc: { $first: "$$ROOT" },
                        ranks: { $push: "$search_type" }
                    }
                },
                {
                    // Stage 4: Apply Reciprocal Rank Fusion (RRF)
                    $addFields: {
                        "doc.rrf_score": {
                            $let: {
                                vars: {
                                    vectorRank: { $indexOfArray: ["$ranks", "vector"] },
                                    textRank: { $indexOfArray: ["$ranks", "text"] }
                                },
                                in: {
                                    $sum: [
                                        { $cond: [{ $ne: ["$$vars.vectorRank", -1] }, { $divide: [1, { $add: [60, "$$vars.vectorRank"] }] }, 0] },
                                        { $cond: [{ $ne: ["$$vars.textRank", -1] }, { $divide: [1, { $add: [60, "$$vars.textRank"] }] }, 0] }
                                    ]
                                }
                            }
                        }
                ...
                { $sort: { "rrf_score": -1 } }, // Sort by the new RRF score
                { $limit: 1 } // Get the single best result
            ]);

            if (searchResults.length > 0) {
                console.log(`✅ Hybrid Search SUCCESS (RRF Score: ${searchResults[0].rrf_score.toFixed(4)})`);
                console.log(`Matched Text: "${searchResults[0].text}"`);
                return res.json(searchResults[0]); // For testing, return the full doc
            }
    ...
    **Your Friendly Answer (in ${language}):**`;
    ...
    //         const embeddingService = await EmbeddingService.getInstance();
    //         console.log(`Creating new knowledge in '${language}': "${text}"`);

    //         // THIS NOW WORKS because 'this' refers to the controller instance
    ...
    //         // const textToEmbed = `${text} ${answer}`;
    //         // const embedding = await embeddingService.getEmbedding(textToEmbed);

    //         // UPDATEDDDD LINE: Include the newly generated keywords in the text to be embedded
    //         const textToEmbed = `${text} ${answer} ${allUniqueKeywords.join(' ')}`;
    //         const embedding = await embeddingService.getEmbedding(textToEmbed);

    //         // IMPORTANT: Make sure the 'Knowledge' model is accessible here.
    //         // It's best to use the model imported at the top of the file.
    ...
    //             id: newKnowledge._id,
    //             generatedKeywords: allUniqueKeywords
    //         });
    ...
    // In knowledgeController.js, inside the EnhancedSmartKnowledgeController class
