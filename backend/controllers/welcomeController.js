...
const submitAnswer = async (req, res) => {
    try {
        ...
        const quiz = await WelcomeQuiz.findById(group.quiz)
        if (!quiz || !quiz.isActive) return res.status(400).json({ error: "Quiz is not active" })
        ...
        const existingAnswer = group.answers.find((a) => a.questionIndex === currentQuestionIndex)
        if (existingAnswer) return res.status(400).json({ error: "Answer already submitted" })
        ...
        // Initial points (temporary, will be updated for multiple_choice)
        let points = 0
        if (isCorrect && currentQuestion.questionType === "open") {
            const maxTime = currentQuestion.timeLimit * 1000
            points = Math.max(100, Math.floor(1000 - (timeTaken / maxTime) * 900))
        }

        // Save answer (without adding points yet if it's multiple_choice)
        const answerObj = {
            questionIndex: currentQuestionIndex,
            answer,
            submittedAt,
        ...
        group.answers.push(answerObj)
        if (currentQuestion.questionType === "open" && isCorrect) {
            group.totalPoints += points
        }
        await group.save()

        // If multiple_choice and correct → Re-rank
        if (isCorrect && currentQuestion.questionType === "multiple_choice") {
            const allGroups = await WelcomeGroup.find({ quiz: quiz._id })
            const correctGroups = allGroups
                .map((g) => {
                    const ans = g.answers.find((a) => a.questionIndex === currentQuestionIndex && a.isCorrect)
                    return ans ? { group: g, answer: ans } : null
                })
                .filter(Boolean)
                .sort((a, b) => a.answer.timeTaken - b.answer.timeTaken)

            // Assign points based on rank (e.g., 1000, 900, 800, ...)
            for (let i = 0; i < correctGroups.length; i++) {
                const rankPoints = Math.max(100, 1000 - i * 100)
                const { group: g, answer } = correctGroups[i]

                const targetAnswer = g.answers.find(
                    (a) => a.questionIndex === currentQuestionIndex && a.isCorrect
                )
                if (targetAnswer) {
                    g.totalPoints -= targetAnswer.points || 0 // remove old
                    targetAnswer.points = rankPoints
                    g.totalPoints += rankPoints
                    await g.save()
                }
            }
        }
